    ...
    for file in os.listdir(path):
        ...
        if source.endswith(".ui"):
            ...
            if not os.access(
                target, os.F_OK) or (
                os.stat(source)[
                    stat.ST_MTIME] > os.stat(target)[
                    stat.ST_MTIME]) or args.force:
    ...
    for file in os.listdir(path):
        ...
        if source.endswith(".qrc"):
            ...
            if not os.access(
                ...
                os.stat(source)[
                    stat.ST_MTIME] > os.stat(target)[
                    stat.ST_MTIME]) or args.force or args.resources:
...
if __name__ == '__main__':
    ...
    if not PYUIC or not PYRCC:
        raise SystemExit(
            "pyuic5 or pyrcc5 could't be found, please install PyQt5 development tools (e.g. pyqt5-dev-tools)")

    cwd = os.path.dirname(os.path.abspath(__file__))
    gns3_path = os.path.abspath(os.path.join(cwd, "../gns3_webclient_pack/"))
    ui_path = os.path.abspath(os.path.join(cwd, "../gns3_webclient_pack/ui"))
